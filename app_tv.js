// --- MISMA CONFIGURACIÓN ROBUSTA DE FETCH ---
...
function tvTab(id) {
    ...
    if(id==='search') {
        // Cargar géneros en TV también si quieres, o dejar solo búsqueda texto
        document.getElementById('tv-search-input').focus();
    }
...
async function tvCargarInicio() {
    ...
    if (data) {
        ...
        document.getElementById('hero-play').onclick = () => {
            const s = hero.slug.split('-').slice(0,-1).join('-');
            tvOpenDetails(s);
        };
        ...
        data.forEach(item => {
            ...
            card.tabIndex = 0;
            card.innerHTML = `<img src="${item.cover}"><div class="info">${item.title}</div>`; // Info visible en TV
            
            card.onclick = () => {
                let s = item.animeSlug || item.slug;
                if(s.includes('-episodio-')) s = s.split('-').slice(0,-1).join('-');
                tvOpenDetails(s);
            };
        ...
        // Cargar fila de favoritos también
...
function renderGridLocal(key, id) {
    renderRowLocal(key, id); // Reutilizamos lógica, CSS maneja el layout
}