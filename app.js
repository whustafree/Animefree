...
window.addEventListener('appinstalled', () => {
    // Si ya se instaló, ocultar botón para siempre
    const installBtn = document.getElementById('btn-install');
...
window.addEventListener('popstate', () => {
    ...
    if (search && search.style.display === 'flex') { search.style.display = 'none'; return; }
    const searchView = document.getElementById('view-search');
    if (searchView && searchView.classList.contains('active')) { cambiarVista('home'); return; }
});
...
async function buscar() {
    ...
    else { grid.innerHTML = '<div style="padding:20px; text-align:center;">No se encontraron resultados.</div>'; }
}
    ...
    if (data && data.servers) {
        ...
        data.servers.forEach((s, i) => {
            ...
            btn.onclick = () => {
                ...
                // VIDEO ESTÁNDAR COMPATIBLE (Sin bloqueo agresivo para que funcione en todos lados)
                // EL BLOQUEO DEBE SER EXTERNO (DNS O BRAVE)
